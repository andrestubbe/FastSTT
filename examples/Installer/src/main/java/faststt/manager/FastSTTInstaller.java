...
public class FastSTTInstaller {
    ...
    public void run() {
        while (true) {
            ...
            System.out.println("  1.  [Model]  Download Whisper Model (GGML)");
            System.out.println("  2.  [Native] Download Whisper DLL (Windows)");
    ...
    private void downloadNativeBinaries() {
        ...
        File nativeDir = new File(projectRoot, "native/include");
        if (!nativeDir.exists()) nativeDir.mkdirs();
        File buildDir = new File(projectRoot, "build");
        if (!buildDir.exists()) buildDir.mkdirs();

        String baseUrl = "https://github.com/ggml-org/whisper.cpp/releases/download/v1.8.1/";
        downloadFile(baseUrl + "whisper-bin-x64.zip", "whisper_temp.zip", projectRoot.getAbsolutePath());
        ...
        try {
            String psCommand = 
                ...
                "Copy-Item -Path 'whisper_extract/Release/*.dll' -Destination 'build/' -Force; " +
                "Copy-Item -Path 'whisper_extract/Release/*.exe' -Destination 'build/' -Force; " +
                "Copy-Item -Path 'whisper_extract/Release/*.dll' -Destination 'native/' -Force; " +
